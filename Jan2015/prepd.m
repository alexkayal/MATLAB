...
for i=2:rows-1
    ...
    if  (rsislow(i)<30 && rsimedium(i)<30 && rsifast(i)<30)
        crosspoints (i)=1; %upcross
    elseif (rsislow(i)>70 && rsimedium(i)>70 && rsifast(i)>70)
        crosspoints (i)=-1; %downcross
...
for i =1:rows
    ...
    % Note: here we we are giving a slight advantage
    % to the loss over the win if the candle contains both. 
    % Number of incidents like that are recorded in "incidents",
    % however, this needs to be fixed, or just finer resolution 
    % needs to be used (like 15 mins or less).
        if (high(i)-orderprice>(TP/10000) && orderprice-low(i)>(SL/10000))
...
% Preparing profit and neural matrix

neuralmatrixprep = [crosspoints profit'];
pcounter = 1;
for i=1:rows
...
% These can be displayed by removing semi colons
